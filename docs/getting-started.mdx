...
For GitHub-based projects, you can enable auto-publish to automatically create pull requests with documentation changes. When enabled, Promptless will create a new PR with suggested documentation updates whenever it detects necessary changes, rather than requiring manual review and PR creation through the dashboard. You can also specify a custom PR template path (e.g., `.github/PULL_REQUEST_TEMPLATE.md`) in the project settings to ensure that Promptless-generated PRs follow your team's PR template format.
