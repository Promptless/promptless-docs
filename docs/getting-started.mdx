...
* Your triggers (e.g. Github, Bitbucket, or Slack). You can trigger Promptless from new pull requests (in Github or Bitbucket) and from the Promptless Slack message action (primarily used in Slack Connect support channels). If you're connecting a source control platform, make sure to give Promptless access to your source code repos that you want Promptless to be triggered from, as well the repo containing your docs.  
...
- Bitbucket: Promptless runs upon opening a PR in any of the source repos you specify in the Project
...
For source control-based projects (GitHub or Bitbucket), you can enable auto-publish to automatically create pull requests with documentation changes. When enabled, Promptless will create a new PR with suggested documentation updates whenever it detects necessary changes, rather than requiring manual review and PR creation through the dashboard.

A new project for a source control PR trigger might look like this:
